    ...
    def __init__(
        ...
        if with_extra_norm:
            ...
            for stage_id in range(len(self.stage_out_channels) - 1):
                self.extra_norm_list.append(
                    nn.BatchNorm2d(self.stage_out_channels[stage_id][-1], eps=NORM_EPS)
                )
...
def nextvit_small(pretrained=False, pretrained_cfg=None, **kwargs):
    model = NextViT(
        stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, with_extra_norm=True, **kwargs
    )

    model.stem[0].conv.stride = (1, 1)
    model.stem[3].conv.stride = (1, 1)
    model.out_channels = [3, 96, 256, 512, 1024]
...
def nextvit_base(pretrained=False, pretrained_cfg=None, **kwargs):
    model = NextViT(
        stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, with_extra_norm=True, **kwargs
    )
...
def nextvit_large(pretrained=False, pretrained_cfg=None, **kwargs):
    model = NextViT(
        stem_chs=[64, 32, 64], depths=[3, 4, 30, 3], path_dropout=0.2, with_extra_norm=True, **kwargs
    )
    ...
    if pretrained:
        model.load_state_dict(torch.load(WEIGHTS["nextvit_large"])["model"], strict=False)
    return model
